...
class MediaPlayer:
    ...
    def repeat_song(self):
        if self.song_list.size() >= 1:
          if not self.repeat_conditions:
              self.repeat_conditions=True
    ...
    def shuffle_song(self):
        if self.song_list.size() >= 1:
          ...
          song_list = list(self.song_list.get('0','end'))
          ...
          for i  ,song in enumerate(song_list):
              self.song_list.insert(i,song)

              if song == song_name:
                  self.song_list.selection_set(i)
                  self.song_list.activate(i)
    ...
    def delete_song(self, command):
        current_song = self.song_list.index('active')
        if command == 'selected':
            if self.song_list.size() > 1:
                if current_song == self.song_list.size() - 1:
                    self.song_list.selection_set(self.song_list.size() - 2)
                    self.song_list.activate(self.song_list.size() - 2)
                else:
                    self.song_list.selection_set(current_song + 1)
                    self.song_list.activate(current_song + 1)

            self.song_list.delete(current_song)
        else:
            self.song_list.delete(0, self.song_list.size() - 1)
